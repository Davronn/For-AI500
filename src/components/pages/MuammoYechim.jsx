      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Problem & Solution — detailed analysis of market gaps and our innovative approach
          </p>
        </div>
        ...
        <section className="mb-20">
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
              <h2 className="text-3xl lg:text-4xl">Muammo</h2>
              <p className="text-muted-foreground">Current challenges in drone training</p>
            </div>
          </div>
            ...
            {problemDetails.map((problem, index) => (
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    <h3 className="text-xl mb-1">{problem.title}</h3>
                    <p className="text-sm text-primary">{problem.titleUz}</p>
                  </div>
                </div>
                <p className="text-muted-foreground mb-3">{problem.description}</p>
                <p className="text-sm text-muted-foreground/70 mb-4">{problem.descriptionUz}</p>
                <div className="pt-4 border-t border-white/10">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">{problem.impactUz}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section>
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
              <h2 className="text-3xl lg:text-4xl">Yechim</h2>
              <p className="text-muted-foreground">Our comprehensive platform approach</p>
            </div>
          </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div>
                        <h3 className="text-xl mb-1">{solution.title}</h3>
                        <p className="text-sm text-primary/80">{solution.titleUz}</p>
                      </div>
                    </div>
                    <p className="text-muted-foreground mb-3">{solution.description}</p>
                    <p className="text-sm text-muted-foreground/70">{solution.descriptionUz}</p>
                  </div>
                  <div className="flex items-center">
                    <div className="w-full p-6 rounded-xl bg-primary/10 border border-primary/20">
                      ...
                      <p className="text-sm mb-1">{solution.benefit}</p>
                      <p className="text-xs text-muted-foreground">{solution.benefitUz}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="mt-20">
          <div className="p-12 rounded-3xl bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20 backdrop-blur-sm">
            <div className="max-w-3xl mx-auto text-center">
              <h3 className="text-2xl lg:text-3xl mb-4">Platform Impact</h3>
              <p className="text-muted-foreground mb-8">
                AgroDrone Academy eliminates cost and safety barriers, making professional drone
                training accessible to everyone in Central Asia. Our AI-powered approach delivers
                certification-ready pilots at 10% of traditional costs.
              </p>
