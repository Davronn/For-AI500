import { Lightbulb, Wrench, Rocket, TrendingUp, CheckCircle, Clock, Target } from 'lucide-react';
  ...
  return (
    <div className="w-full py-20">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
          ...
          <div className="p-8 rounded-3xl bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20 backdrop-blur-sm text-center">
            <div className="inline-flex items-center gap-3 mb-4">
              ...
              <div className="text-left">
                <div className="text-sm text-primary">Current Stage</div>
                <div className="text-2xl">MVP / Prototype</div>
              </div>
            </div>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              We are currently in Phase 3 with a fully functional MVP. Beta testing is underway
              with real users in the Central Asian agricultural sector.
            </p>
            <p className="text-sm text-muted-foreground/70 mt-2">
              Hozirda biz to\'liq ishlaydigan MVP bilan 3-bosqichdamiz. Beta test Markaziy Osiyo
              qishloq xo\'jaligi sektorida real foydalanuvchilar bilan ketmoqda.
            </p>
          </div>
        </section>
        ...
        <section className="mb-20">
          <div className="mb-12">
            <h2 className="text-3xl lg:text-4xl mb-4">Development Timeline</h2>
            <p className="text-muted-foreground">
              Ishlab chiqish vaqt jadvali — from concept to launch
            </p>
          </div>
              ...
              {phases.map((phase, index) => (
                <div key={index} className="relative pl-16 md:pl-24">
                  ...
                  >
                    ...
                    <h3 className="text-2xl mb-1">{phase.title}</h3>
                    <p className="text-primary/80 mb-4">{phase.titleUz}</p>
                    <p className="text-muted-foreground text-sm mb-2">{phase.period}</p>
                    <p className="text-muted-foreground/70 text-sm mb-6">{phase.periodUz}</p>

                    <div className="grid md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="text-sm mb-3 text-muted-foreground">Milestones</h4>
                        <ul className="space-y-2">
                          {phase.milestones.map((milestone, idx) => (
                            <li key={idx} className="flex items-start gap-2 text-sm">
                              <div
                                className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                                  phase.status === 'completed' || milestone.includes('✅')
                                    ? 'bg-primary'
                                    : 'bg-muted-foreground'
                                }`}
                              ></div>
                      ...
                      <div>
                        <h4 className="text-sm mb-3 text-muted-foreground">O'zbek tilida</h4>
                        <ul className="space-y-2">
                          {phase.milestonesUz.map((milestone, idx) => (
                            <li key={idx} className="flex items-start gap-2 text-sm">
                              <div
                                className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                                  phase.status === 'completed' || milestone.includes('✅')
                              ...
                              ></div>
                              <span className="text-muted-foreground/70">{milestone}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
        ...
        <section>
          <div className="mb-12">
            <h2 className="text-3xl lg:text-4xl mb-4">Next Steps</h2>
            <p className="text-muted-foreground">
              Keyingi qadamlar — immediate priorities for the coming months
            </p>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-primary/80">{step.titleUz}</p>
                  </div>
                  <div
